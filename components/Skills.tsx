  ...
  return (
    <section id="skills" className="scroll-mt-24 border-t border-border-muted py-20 sm:py-28">
      <Container>
        ...
        <StaggerContainer staggerDelay={0.08} className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {skillGroups.map((group) => {
            const Icon = group.icon;
            return (
              <FadeIn key={group.title} direction="up" distance={20} className="h-full">
                <div
                  className="h-full rounded-xl border border-border-muted bg-surface-raised p-5 shadow-3 transition-colors duration-fast hover:border-text-secondary/40"
                >
